...
// let unique_event = new Set();


// Add a function to validate generated code
function validateAndFormatCode(code) {
    ...
    try {
        ...
        if (parseResult.errors && parseResult.errors.length > 0) {
            console.log("Parse warnings found:", parseResult.errors);
        }
...
function needsOwnable(functionCode) {
    // Check if the function uses onlyOwner modifier
    return functionCode.includes('onlyOwner');
}
...
function getCalledFunctionNames(jsCode) {
    // Skip modifier declarations and only match actual function calls
    // Look for pattern where function name is followed by parentheses but not by curly braces
    const functionCallRegex = /\b([a-zA-Z_]\w*)\s*\([^{]*\)/g;
    let matches = [...jsCode.matchAll(functionCallRegex)].map(match => match[1]);
    
    // Remove known modifiers and function declarations from matches
    const knownModifiers = new Set();
    modifierToParent.forEach((_, modifierName) => {
        knownModifiers.add(modifierName);
    });
    
    // Filter out modifiers and duplicates
    matches = matches.filter(match => 
        !knownModifiers.has(match) && 
        !match.startsWith('modifier') &&
        match !== 'require' &&
        match !== 'assert' &&
        match !== 'revert'
    );
...
// function hasConstructor(ast, contractName) {
//     let foundConstructor = false;

//     parser.visit(ast, {
//         ContractDefinition(node) {
//             // Check if this node matches the contract of interest
//             if (node.name === contractName) {
//                 // Look through the subNodes for a constructor
            ...
            if (node.name === contractName) {
                for (const subNode of node.subNodes) {
                    if (
                        ...
                        constructorInfo.found = true;
                        // Check if constructor has any parameters
                        constructorInfo.hasParameters = subNode.parameters && 
...
function handlePayableAddressType(typeName) {
    // Special handling for payable address type
    if (typeName.name === 'address' && typeName.stateMutability === 'payable') {
        return 'address payable';
    }
    return typeName.name;
}

const parentConstructorDefaults = {
    "ERC20": `"Alpha D.O.M", "ADOM"`,
    "ERC721": `"MyNFT", "MNFT"`,
    "Ownable": "msg.sender",
    // add other parent contracts as needed
};
...
function generateFacetFiles(function_array) {
	...
	for (let index = 0; index < function_array.length; index++) {
		const functionName = function_array[index];
		...
		let functionInfo = functions_information.get(function_array[index]); //-----------harsh

		if (functionInfo === undefined) {
            console.warn(`WARNING: No information found for function ${functionName}`);
            continue;  // Skip this function
        }
		// add event to allEvents only if it is unique
		if(!unique_event.has(functionInfo.events)) {
			unique_event.add(functionInfo.events);
			allEvents += functionInfo.events;
		}
	
		// allEvents += functionInfo.events
		console.log("All Events: *************************", allEvents);
		allFunctionCode += functionInfo.rePlacedCode

		if (needsOwnable(functionInfo.rePlacedCode)) { // ------------------------------------------------ added for Ownable
            facetParents.add('Ownable');
        }
	}
	console.log('\n');
	console.log("All Events: ", allEvents);
	...
	let facetContractType = "contract";
	...
	console.log("After Sorting: ", Array.from(sortedList));


	// let inheritanceList = [];

	// if (sortedList.includes('IERC20')) {
	// 	inheritanceList.push('IERC20');  // IERC20 never has constructor so we can add directly
	// }


	// if (sortedList.includes('ERC20')) {
	// 	// Check if ERC20 has a constructor
	// 	const erc20HasConstructor = hasConstructor(ast, 'ERC20');
	// 	if (erc20HasConstructor.hasParameters) {
	// 		inheritanceList.push('ERC20("Alpha D.O.M", "ADOM")');
	// 	} else {
	// 		inheritanceList.push('ERC20');
	// 	}
	// }
	
	// if (sortedList.includes('Ownable')) {
	// 	const ownableHasConstructor = hasConstructor(ast, 'Ownable');
	// 	if (ownableHasConstructor.hasParameters) {
	// 		inheritanceList.push('Ownable(msg.sender)');
	// 	} else {
	// 		inheritanceList.push('Ownable');
	// 	}
	// }

	// const isClause = inheritanceList.length > 0 ? `is ${inheritanceList.join(', ')}` : '';

	const inheritanceList = [];
    for (const parent of sortedList) {
        const constructorInfo = hasConstructor(ast, parent);
        if (constructorInfo.found && constructorInfo.hasParameters) {
            const defaultParams = parentConstructorDefaults[parent] || "";
            if (defaultParams) {
                inheritanceList.push(`${parent}(${defaultParams})`);
            } else {
                console.warn(`Warning: No default parameters provided for ${parent}. Adding without parameters.`);
                inheritanceList.push(`${parent}`);
            }
        } else {
            inheritanceList.push(parent);
        }
    ...
    ${facetContractType} ${function_array[0]}Facet ${isClause} {
        ${usingDeclaration}
        ${allModifiers}
        ${allEvents}
        ${allFunctionCode}
	...
	// try {
    //     const formattedCode = validateAndFormatCode(fileCode);
    //     fs.writeFileSync(fileName, formattedCode);
    // } catch (error) {
    //     console.error(`Error writing file ${fileName}:`, error);
    //     // Write unformatted code as fallback
    //     fs.writeFileSync(fileName, fileCode);
    // }
...
function distributeFunctions() {
	...
	if(foundContract === false) {
		throw "Contract with the same name as filename not found";
	}
			...
			node.subNodes?.forEach((subNode) => {
				if (subNode.type == "StateVariableDeclaration") {
					...
					if (typeName.type == "ArrayTypeName") {
						...
						const baseType = typeName.baseTypeName.type === "UserDefinedTypeName" 
					...
					varType = baseType +
						...
						const valueTypeName = typeName.valueType.type === "UserDefinedTypeName" 
						? typeName.valueType.namePath 
						: typeName.valueType.name;
				
					varType += typeName.keyType.name + " => " + valueTypeName;
					varType += ")".repeat(num_mappings);
					} else if (typeName.type == "UserDefinedTypeName") {
						varType = typeName.namePath;
					}
					// else if (typeName.stateMutability === "payable" && typeName.name === "address") {
					// 	varType = "address payable";
					// }
					else {
					...
					if (subNode.variables[0].isDeclaredConst) {
						if (typeName.stateMutability == "payable") {
							constants += `address payable constant ${varName}`;
						} else {
	...
	// in distributeFunctions() or a function it calls:
// let globalEvents = new Map(); // key: eventName, value: array of code for definitions

// ast.children.forEach((node) => {
//   if (node.type === "ContractDefinition") {
//     node.subNodes?.forEach((subNode) => {
//       if (subNode.type === "EventDefinition") {
//         const startLine = subNode.loc.start.line;
//         const endLine   = subNode.loc.end.line;
//         const lines = sourceCode.split("\n").slice(startLine - 1, endLine);
//         const eventDefinitionCode = lines.join("\n");

//         // If you might have duplicates, check if we've already stored it
//         if (!globalEvents.has(subNode.name)) {
//           globalEvents.set(subNode.name, eventDefinitionCode);
//         }
//       }
//     });
//   }
		...
		if (node.type === "ContractDefinition" && node.name == CONTRACT_NAME) {
			...
			node.subNodes.forEach((subNode) => {
				...
				if (
					...
					console.log(`Generating file for function++++++++++++++++++++++++++++++++++ ${subNode.name}`);
...
function generateFileContents(node, sourceCode) {
	...
	if (node.type === "ModifierDefinition") {
        modifierToParent.set(node.name, CONTRACT_NAME);
    }
	...
	const emitRegExp = /emit\s+([a-zA-Z0-9_]+)\s*\(([\s\S]*?)\)\s*;/g;
	var visited = new Map();
	...
	while ((emitMatch = emitRegExp.exec(rePlacedCode)) !== null) {
		...
		// if (!unique_event.has(eventName)) {
        //     unique_event.add(eventName);
	...
	// 		// Now we pull from "globalEvents"
	// 		if (globalEvents.has(eventName)) {
	// 			const eventDefinition = globalEvents.get(eventName);
	...
	while ((match = functionCallPattern.exec(functionCode)) !== null) {
		...
		if (!modifierToParent.has(functionName)) {
...
function generateLibraryFile(sourceCode,declrations){
	...
	//     try {
    //     const formattedCode = validateAndFormatCode(libCode);
    //     fs.writeFileSync(`${dirName}/${LIB_NAME}Lib.sol`, formattedCode);
    // } catch (error) {
    //     console.error(`Error writing library file:`, error);
    //     // Write unformatted code as fallback
    //     fs.writeFileSync(`${dirName}/${LIB_NAME}Lib.sol`, libCode);
    // }
