...
👁‍🗨Eye of Etherum is a Telegram AI bot that tracks every ETH transaction in real time to provide fast notifications of upcoming project launches

Some of its features are
- Instant alerts when new token contracts are deployed and when liquidity is added to Uniswap v2/v3
- LP monitoring including removal detection from pending transactions
- Sentiment analysis based on number of transactions, unique transactions, frequency, block distance from launch
- Contract discovery and source code processing
- Backed by a database that records wallet interactions to warn against previous nefarious acts
...
contract EyeOfEthereum is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "Eye of Ethereum";
